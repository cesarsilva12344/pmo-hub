// Usa as globais window.PMO_CONFIG, window.Auth, window.PMO_Metrics
const supabaseClient = window.supabase.createClient(window.PMO_CONFIG.SUPABASE_URL, window.PMO_CONFIG.SUPABASE_KEY);
...
async function init() {
    try {
        ...
        // Verifica admin usando a global Auth
...
// ... (Resto das funções de CRUD mantidas idênticas à v15.0, apenas garantindo uso de window.* onde necessário)
...
function renderAdminView() {
    ...
    projects.forEach(p => {
        ...
        if(models[m] !== undefined) models[m]++; else models['tradicional']++;
    });
...
function openProject(id) {
    ...
    const typeLabel = document.getElementById('sb-proj-type'); if(typeLabel) typeLabel.innerText = model.toUpperCase();
    
    const navGantt = document.getElementById('nav-gantt');
    const navKanban = document.getElementById('nav-kanban');
    if (model === 'agil') { if(navGantt) navGantt.classList.add('hidden'); if(navKanban) navKanban.classList.remove('hidden'); } 
    else { if(navGantt) navGantt.classList.remove('hidden'); if(navKanban) navKanban.classList.add('hidden'); }
    showTab('dashboard');
}

// Funções de Renderização e CRUD (Simplificadas para caber)
...
function renderRiskMatrix() {
    ...
    p.risks.forEach((r, idx) => { 
        ...
        list.innerHTML += `<div class="flex justify-between items-center p-2 bg-slate-50 border rounded text-xs"><div><span class="font-bold ${color}">[${score}]</span> ${r.desc}</div><button onclick="turnIssue(${idx})" class="text-[10px] bg-white border px-2 py-1 hover:bg-red-50 text-red-500">Virar Issue</button></div>`; 
        const cellId = `cell-${r.prob}-${r.imp}`; const cell = document.getElementById(cellId); if(cell) cell.innerHTML += `<div class="risk-dot" title="${r.desc}">${idx+1}</div>`; 
    });
}
...
function turnIssue(idx) { if(!confirm("Virar Issue?")) return; const p = projects.find(x => x.id === currentProjId); p.risks[idx].type = 'issue'; p.risks[idx].desc = "[ISSUE] " + p.risks[idx].desc; save(); renderRiskMatrix(); }
...
function addKanban(col) { const t = prompt("Tarefa:"); if(t) { const p = projects.find(x => x.id === currentProjId); if(!p.kanban) p.kanban = { todo: [], doing: [], done: [] }; p.kanban[col].push(t); save(); renderKanban(); } }
function delKanban(col, idx) { const p = projects.find(x => x.id === currentProjId); p.kanban[col].splice(idx, 1); save(); renderKanban(); }

// Standard Functions (Mantidas)
...
async function createProject() { const name = document.getElementById('np-name').value; if (!name) return alert("Nome obrigatório"); const model = document.getElementById('np-model').value; const client = document.getElementById('np-client').value || 'Geral'; const btn = document.querySelector("button[onclick='createProject()']"); btn.innerText = "Salvando..."; btn.disabled = true; const newId = Date.now(); const newP = { id: newId, name, pm: document.getElementById('np-pm').value || 'Gerente', client, logo: "fas fa-building", model: model, fin: { setup: 0, sustain: 0 }, info: { hours_plan: 0, hours_real: 0, costmode: 'CAPEX' }, sCurve: Array(12).fill(0).map((_, i) => ({ month: months[i], plan: 0, real: 0 })), gantt: [], kanban: { todo: [], doing: [], done: [] }, risks: [], canvas: { pain: '', obj: '' }, stakeholders: [], raci: [], checks: {} }; projects.push(newP); try { await supabaseClient.from('projects').upsert({ id: newId, content: newP }); closeModal('modal-project'); goToPortfolio(); renderFilters(); document.getElementById('np-name').value = ''; } catch (err) { alert("Erro ao salvar."); } finally { btn.innerText = "Criar"; btn.disabled = false; } }
async function deleteProject(id) { if(!confirm("Excluir?")) return; try { await supabaseClient.from('projects').delete().eq('id', id); projects = projects.filter(p => p.id !== id); renderPortfolio(); if(currentProjId === id) goToPortfolio(); } catch (err) { alert("Erro."); } }
// Helper functions
